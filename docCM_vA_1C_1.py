...
def getVirtualAuthors(docsInClass):
    ...
    for c,d in docsInClass.iteritems():
        for i in range(0,len(d),30):
            vList.append(' '.join([a for a in d[i:i+30]]))
            vC.append(c)
    return vList, vC
...
# ## ! when a test sample is given it does NOT get the list of probabilities from all classifiers ; it does NOT get to know whether it belongs to a class or not; all bulk tests are given to a model and it is tested
