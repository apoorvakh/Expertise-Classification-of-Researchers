...
def benchmark(clf):
    ...
    for classC in categories:
        ...
        if True: #or opts.print_report :
            ...
            print(metrics.classification_report(new_y_test, pred, target_names=['-1',classC,'-1']))
    ...
    for d in range(len(y_test)):
        ...
        for c in cumPred.keys():
            docPClasswise[d][c] = [p[d][1] for p in cumPPred[c]]
    ...
    print("Prediction : ")
    for i,d in enumerate(docPClasswise[:10]):
        print('doc',i,d)
    for i,d in enumerate(docP[:10]):
