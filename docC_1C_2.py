...
GdocClass .add_nodes_from([str(i+1) for i in range(len(y_test))])
GClass = nx.Graph()
GdocClass.add_nodes_from(categories)
print(GClass.nodes())
'''for i in categories:
...
def benchmark(clf):
    ...
    for classC in categories:
        ...
        for i in range(len(pred)):
            if pred[i]!='-1':
                GdocClass .add_edge(str(i+1),classC)
...
nx.write_gml(GdocClass ,"1CdocClassTest2.gml")
nx.write_gml(GClass ,"1CClassTest2.gml")
